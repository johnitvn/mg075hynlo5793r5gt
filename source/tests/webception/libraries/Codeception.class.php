...
namespace webception\libraries;
...
class Codeception {
    ...
    public function loadTests() {
        ...
        foreach ($this->config['tests'] as $type => $active) {
            ...
            $files = new \RecursiveIteratorIterator(
                    new \ RecursiveDirectoryIterator("{$this->config['paths']['tests']}" . $this->config['DS'] . "{$type}" . $this->config['DS'], \FilesystemIterator::SKIP_DOTS), \RecursiveIteratorIterator::SELF_FIRST
            );
    ...
    public function getRunResponse($type, $hash) {
        ...
        // and it won't be run.
        if (!$this->ready())
            $response['message'] = 'The Codeception configuration could not b
                e loaded.';
